...
In addition, other developers have occasionally contributed features of different degree of complexity (ranging from small, incremental changes to large features over a few weeks of work).

Looking at the entire process, I can safely say that the process of onboarding has been smooth and fast, especially for internal hires from different teams.

There are of course lessons learnt from this process and this is exactly what this blog post is about.

## The organization

PSPDFKit is a distributed company where everyone works remotely. The PSPDFKit Server team includes people from different countries with a strong timezone overlap, but no fixed working hours.
...
Apart from work on platforms where there’s strong incentives to standardize on a specific tool (e.g. XCode for iOS development), people pick their tools based on personal preference.

A quick informal survey of the Server Team reveals that people use Vim, Emacs, VSCode, and JetBrains IntelliJ, prevalently on MacOS.
...
We start with small, incremental features as they have a very high completion success rate. For internal hires, we try and choose something that is connected to a use case the person is already familiar with from previous work.

A more experienced team member is available for general guidance, so that the resulting piece of work includes all needed components: implementation, tests and documentation.

On a technical level, we try to avoid pieces of work that introduce more than one key concept: if the person is not familiar with immutable data structures, we avoid assigning a feature that would also introduce concurrency patterns.
...
Once the basics are clear, we build up by looking at some codebase components in order to address topics like error handling, logging, production hardening, and recovery.
...
- We make a clear distinction between company and community conventions, as they may differ. This way, when the developer reads other code on their own they can compare and contrast approaches (often providing constructive criticism on the way we do things).
- We slow down as needed during pairing sessions focused on learning, allowing ourselves to diverge from the initial session goal if the person wants to drill into a different topic. Maintaining interest and enthusiasm is more important in the early stages compared to sticking to a pre-defined plan.
...
Understanding pattern matching comes in the first hour of reading existing code, with more time needed to develop a sensibility around when to use a single function head with an internal case statement versus multiple function heads with specific pattern matching clauses.
...
1. Should this value be configurable?
2. What about changing the value at runtime?
3. How does configuration interact with building a Docker image and later on the container runtime?
...
Common issues include hitting some light race conditions in the form of tests having unexpected results (due to configuration values being changed non-deterministically). It's a great opportunity to reinforce the idea that as much as possible we should strive to write deterministic code that reads configuration as early as possible and passes relevant values down the line.
...
The application depends on a few stateful components: metadata store, assets store, different layers of caching (both filesystem and memory).
...
We found very little use for meta-programming, so we don't make it an explicit part of the onboarding process. Most of the times, people look into it by themselves when they start exploring the internals of dependencies like Phoenix and Ecto.
...
One common observation is that compared to other languages, our application (like other Elixir applications I worked with in the last few years) is built on very few architectural patterns, especially outside concurrency-related areas (which very often implement explicit behaviours).

Most of the times, we simply take care of separating modules that perform side effects (e.g. writing/reading files, http calls, database interaction) from modules which only manipulate data.
...
As we don't operate the software we write, we need to take extra care with error handling, because as best as we can we want to provide error messages that are simple(r) to understand for our customers (who are not Elixir developers).
